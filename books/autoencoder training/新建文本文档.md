...
由于VAE-GAN整个training step涉及Generator和Discriminator混合优化，我们需要依次compute_loss、manual backward、clip gradients、step optimizer，还需要显式的 toggle/untoggle optimizer（requires grad）避免梯度污染，这里我们未参考 diffuser training example——偶数梯度累积步训练 VAE(Generator)，奇数步训练 Disc，而是按照标准GAN训练流程，在同一个 `training_step` 中依次更新 Generator（VAE）和 Discriminator。
...
有个优化的点，在validation step中，我们需要使用ema weight进行fwd并评估多项metrics，结束后切回原始权重，大致的流程如下：
